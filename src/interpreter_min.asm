...
; --- CONSTANTS (must be before data using them) ---
    MAX_TOKENS      EQU 32
    MAX_TOKEN_LEN   EQU 32
    MAX_VARS        EQU 10

.data
; --- IF COMMAND SUPPORT DATA ---
ifCmd BYTE "if",0
isCmd BYTE "is",0
greaterthanCmd BYTE "greaterthan",0
timesCmd BYTE "times",0
ifErrMsg BYTE "Error: syntax is 'if <expr> is greaterthan <expr> print <message> <count> times'",0Dh,0Ah,0
tokensTmp BYTE MAX_TOKENS * MAX_TOKEN_LEN DUP(0)
    ...
    helpText    BYTE "Commands:",0Dh,0Ah
                ...
                BYTE "  print <value> <N> times - Print value N times",0Dh,0Ah
                ...
                BYTE "  if <expr> is greaterthan <expr> print <msg> <count> times",0Dh,0Ah
                BYTE "                       - Conditional print if expr1 > expr2",0Dh,0Ah
...
main PROC
    ...
    call GetToken
    mov esi, eax
    mov edi, OFFSET timesCmd
    call StrEq
    cmp eax, 0
    jne ifErr
    ; If left > right, print message count times
    cmp ebx, ecx
    jle ifDone
    mov esi, edi ; count
    mov edi, edx ; message ptr
ifPrintLoop:
    cmp esi, 0
    jle ifDone
    mov edx, edi
...
ifErr:
    ...
    ; if (must be after all standard commands)
    mov esi, ebx
    mov edi, OFFSET ifCmd
    call StrEq
    cmp eax, 0
...
doPrint:
    ...
    ; Check for 'print <value> <N> times' pattern
    cmp tokenCount, 4
    jl printSimple
    ; tokenCount >= 4, check if last token is 'times'
    mov eax, tokenCount
    sub eax, 1
    call GetToken
    mov esi, eax
    ...
    call ParseOperandValue
    jc printSimple
    mov ecx, eax ; repeat count
    ; print value/expression (tokens 1..tokenCount-3)
    mov eax, 1
    call GetToken
    mov esi, eax
    call ParseOperandValue
